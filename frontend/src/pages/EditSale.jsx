...
import { ShoppingCart, Printer, Check, Trash2, ArrowLeft } from 'lucide-react';
...
const EditSale = () => {
    ...
    async function loadData() {
      try {
        const [cRes, pRes, dRes, sRes] = await Promise.all([
          ...
          api.get('/sales') // We fetch all to filter. In large scale, a specific /sales/:id endpoint would be better.
        ]);
        setClients(Array.isArray(cRes.data) ? cRes.data : []);
        setProducts(Array.isArray(pRes.data) ? pRes.data : []);
        setDeliveryPeople(Array.isArray(dRes.data) ? dRes.data : []);
        
        const allSales = Array.isArray(sRes.data) ? sRes.data : [];
        const saleToEdit = allSales.find(s => String(s.id) === String(id));
        
        if (saleToEdit) {
          setClientId(saleToEdit.client_id || (saleToEdit.client ? saleToEdit.client.id : ''));
          setDeliveryType(saleToEdit.delivery_type || 'DELIVERY');
          setDeliveryPersonId(saleToEdit.delivery_person_id || '');
          
          // Pre-populate cart from sale items
          if (saleToEdit.items && saleToEdit.items.length > 0) {
            const reconstructedCart = saleToEdit.items.map(saleItem => {
              const productFull = (pRes.data || []).find(p => p.id === saleItem.product.id) || saleItem.product;
              return {
                product: productFull,
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      console.error('Erro de rede ou na API ao atualizar venda:', error);
      alert(error.response?.data?.error || 'Erro ao modificar os números da venda.');
    } finally {
    ...
    <div>
      <div className="no-print">
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
          <button className="btn btn-outline" style={{ padding: '0.4rem 0.8rem' }} onClick={() => navigate('/')}><ArrowLeft size={18} /> Voltar</button>
          <h2 style={{ margin: 0 }}>Revisar Venda #{id}</h2>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          ...
          <div>
            <div className="card" style={{ marginBottom: '1.5rem' }}>
              <h4 style={{ marginBottom: '1rem', color: 'var(--primary)' }}>1. Atualizar Cliente</h4>
              <SearchableSelect 
          ...
          <div className="card">
            ...
            <button className="btn btn-primary" style={{ width: '100%', padding: '1rem', cursor: isSubmitting ? 'not-allowed' : 'pointer' }} onClick={handleCheckout} disabled={cart.length === 0 || isSubmitting}>
              {isSubmitting ? (
                 ...
                 <><Check size={20} /> Autenticar e Sobrescrever Venda</>
              )}
            </button>
          </div>
